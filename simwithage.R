...
# m <- lm(FINAL ~ PB + I(PB^2) + years2marathon, data = df[is.na(df$TWINS),])
m <- lm(FINAL ~ PB + years2marathon, data = df[is.na(df$TWINS),])
...
p <- ggplot(filter(hahner_df_simwithage, !is.na(time_diff)), aes(time_diff, fill = time_diff < 60 )) + 
     ...
     annotate("text", y = text_y*max_y_t, x = text_x*max_x_t, label = paste0("< 1 minute:  ", less_than_min), size = 5, col = "red") +
     theme_bw()
print(p)
dev.off()
