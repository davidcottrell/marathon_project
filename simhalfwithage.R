...
hahner_df_simhalfwithage <- data_frame(anna_final = anna_sim$YHAT,
...
less_than_hahner <- sum((hahner_df_simhalfwithage$time_diff) <= 1, na.rm = T ) 
less_than_min <- sum((hahner_df_simhalfwithage$time_diff) < 60, na.rm = T )
consecutive <- sum((hahner_df_simhalfwithage$rank_diff) <= 1, na.rm = T )
...
p <- ggplot(filter(hahner_df_simhalfwithage, !is.na(time_diff)), aes(time_diff, fill = time_diff < 60 )) + 
  geom_histogram(breaks = seq(0,max_x_t, 30), colour = "black") +
  xlab("\nTime difference, in seconds") +
  ylab("Count\n") +
  ylim(0, max_y_t_half) + 
  scale_fill_manual(values = c(NA, "red"), guide = F) +
  annotate("text", y = text_y*max_y_t_half, x = text_x*max_x_t, label = paste0("< 1 minute:  ", less_than_min), size = 5, col = "red") +
  theme_bw()
print(p)
dev.off()

pdf("plots/simulated_rank_half_with_age.pdf", height = 5, width = 5)
p2 <- ggplot(filter(hahner_df_simhalfwithage, !is.na(rank_diff)), aes(rank_diff, fill = rank_diff <= 1 )) + 
...
cat("The average simulated time between them was ", round(mean(hahner_df_simhalfwithage$time_diff/60, na.rm = T), 2), " minutes\n", sep = "")
