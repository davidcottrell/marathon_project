...
hahner_df_simhalf <-  data_frame(anna_final = anna_sim$YHAT,
...
less_than_hahner <- sum((hahner_df_simhalf$time_diff) <= 1, na.rm = T ) 
less_than_min <- sum((hahner_df_simhalf$time_diff) < 60, na.rm = T )
consecutive <- sum((hahner_df_simhalf$rank_diff) <= 1, na.rm = T )
...
p <- ggplot(filter(hahner_df_simhalf, !is.na(time_diff)), aes(time_diff, fill = time_diff < 60 )) + 
...
p2 <- ggplot(filter(hahner_df_simhalf, !is.na(rank_diff)), aes(rank_diff, fill = rank_diff <= 1 )) + 
...
cat("The average simulated time between them was ", round(mean(hahner_df_simhalf$time_diff/60, na.rm = T), 2), " minutes\n", sep = "")
