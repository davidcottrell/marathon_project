...
regression lines.  The ticks along the x-axis in both plots indicate times (either
...
half marathon split times. As one might expect, a runner's time halfway through the race is a
fairly strong predictor of their finishing time at the end of the race.  Here we see that the relationship
between the twins' halfway time and their finishing time is similar to that
of the other runners.  The two points representing the Hahners are
in the middle of the distribution of points and therefore imply that the twins do not seem to have slowed down in any unusual way. 
...
Figure \ref{fig:residuals} gives a visualization of how unusual, or not,
...
left and right panels of the figure display the studentized residuals derived from the polynomial
regressions in Figure \ref{fig:scatter}'s panels, respectively.  Hence, we
compute the residuals from Figures \ref{fig:45degreeplot} and
\ref{fig:45degreeplot_half}, Studentize them, and arrange the result from least to most
along the x-axis in Figure \ref{fig:residuals} to display their cumulative distribution.
...
the other at the 93rd.  While they are in the tail end of the distribution, they are not the
major outliers that would lead us to reject the twins' claim.  
Moreover, the residuals for the Kim twins are similarly not
...
them than the difference in their personal bests - more than any other pair of runners with a similar
